...
function vecinos(lat, lon, n, excluirId) {
  let radio = 12, cand = [];
  while (radio <= 400) {
    cand = REJILLA.cerca(lat, lon, radio);
    if (cand.length >= n + (excluirId ? 1 : 0) + 4) break;
    radio *= 2;
  }
...
/** Trazado y costo estimado de un tendido de fibra hasta un nodo. */
function analizarFO(candLat, candLon, nodo, P) {
  const km = haversine(candLat, candLon, nodo.lat, nodo.lon);
  const urbano = nodo.morfologia === 'URBANO';
  const sinuosidad = urbano ? P.sinuosidadUrbana : P.sinuosidadRural;
  const rutaKm = km * sinuosidad;
  ...
  if (rutaKm <= P.foVerdeKm && fo.score >= 60) {
    grade = 'ok';
    why = `${fmt(rutaKm, 2)} km de tendido hasta ${nodo.id}, nodo con probabilidad ${fo.nivel} de fibra.`;
  } else if (rutaKm <= P.foVerdeKm && fo.score >= 35) {
    grade = 'warn';
    why = `Tendido corto (${fmt(rutaKm, 2)} km), pero hay que confirmar fibra en ${nodo.id} (probabilidad ${fo.nivel}).`;
  } else if (rutaKm <= P.foAmbarKm && fo.score >= 35) {
    grade = 'warn';
    why = `${fmt(rutaKm, 2)} km de tendido: viable con obra civil, sujeto a permisos y a confirmar fibra en ${nodo.id}.`;
  } else if (rutaKm <= P.foAmbarKm) {
    grade = 'bad';
    why = `${fmt(rutaKm, 2)} km hasta un nodo con probabilidad ${fo.nivel} de fibra: costo alto frente al beneficio.`;
  } else {
    grade = 'bad';
    why = `${fmt(rutaKm, 2)} km de tendido superan el umbral de ${P.foAmbarKm} km. Resolver por radio o por un tercero.`;
  }
  return { km, rutaKm, sinuosidad, costo, costoKm, urbano, fo, grade, why, nodo };
}
...
function analizarSitio(cand, P) {
  const cerca = vecinos(cand.lat, cand.lon, P.nNodos, cand.id);
...
function parsearLote(texto) {
  ...
  lineas.forEach((linea, i) => {
    ...
    /* Sólo un campo: debe ser un código de sitio existente. */
    if (campos.length === 1) {
      const s = byId.get(campos[0].toUpperCase());
      if (s) { filas.push({ nombre: s.id, lat: s.lat, lon: s.lon, alt: s.alt, ref: s, linea: i + 1 }); }
      else errores.push(`Línea ${i + 1}: «${bruto}» no es un código de la red ni trae coordenadas.`);
      return;
    }
...
const Mapa = (function () {
  ...
  function grilla(t) {
    ...
    for (let la = Math.ceil(la0 / paso) * paso; la <= la1; la += paso) {
      ...
      ctx.globalAlpha = 0.22; ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke();
      ctx.globalAlpha = 1;
      if (y < H - 92) ctx.fillText(`${la.toFixed(paso < 1 ? 2 : 0)}°`, 4, y - 3);  // la leyenda ocupa la esquina
    }
    const lo0 = lonDe(0), lo1 = lonDe(W);
    for (let lo = Math.ceil(lo0 / paso) * paso; lo <= lo1; lo += paso) {
      const x = Math.round(px(lo)) + 0.5;
      ctx.globalAlpha = 0.22; ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke();
      ctx.globalAlpha = 1; ctx.fillText(`${lo.toFixed(paso < 1 ? 2 : 0)}°`, x + 3, H - 5);
    }
  }

  function escalaGrafica(t) {
    const kmPorPx = (haversine(latDe(H / 2), lonDe(0), latDe(H / 2), lonDe(100))) / 100;
    const objetivo = [1, 2, 5, 10, 20, 50, 100, 200, 500, 1000]
      .find((k) => k / kmPorPx > 60) || 1000;
    const largo = objetivo / kmPorPx;
    const x = W - largo - 16, y = H - 22;
    ...
    ctx.fillText(`${objetivo} km`, x + largo / 2, y - 6);
    ctx.textAlign = 'left';
  }
  ...
  function pintar() {
    ...
    for (const s of RED) {
      ...
      ctx.globalAlpha = s.techMask & 8 ? 0.95 : s.techMask & 4 ? 0.6 : 0.34;
    ...
    capas.enlaces.forEach((e) => {
      ...
      if (e.medio === 'fo') ctx.setLineDash([5, 3]);
      ctx.beginPath();
  ...
  function init(canvas, alHacerClic) {
    ...
    cv.addEventListener('wheel', (e) => {
      ...
      escala = Math.max(2000, Math.min(9e6, escala * k));
    ...
    mq.addEventListener && mq.addEventListener('change', pintar);
...
function ejecutar() {
  ...
  if (modo === 'lote') {
    ...
    ESTADO.resultados = filas.map((f) => {
      ...
      if (f.latB != null) {
        const A = { nombre: `${f.nombre} · A`, lat: f.lat, lon: f.lon, alt: f.alt || P.alturaCandidato };
        const B = { nombre: `${f.nombre} · B`, lat: f.latB, lon: f.lonB, alt: P.alturaCandidato };
        const mw = analizarMMOO(A, B, P, null);
        return { tipo: 'enlace', nombre: f.nombre, P, A, B, mw,
...
function renderSitio(res) {
  ...
  $('#veredictos').innerHTML = `
    <article class="verdict fo" data-grade="${f.grade}">
      ...
      <div class="kv">
        ...
        <div><span class="k">Sinuosidad</span><span class="v">×${fmt(f.sinuosidad, 2)}</span></div>
  ...
  Mapa.fijar({
    ...
    enlaces: [
      { a: cand, b: f.nodo, medio: 'fo', principal: true },
...
function filaNodo(r, cand, i) {
  ...
  return `<tr data-nodo="${esc(s.id)}" class="${i === 0 ? 'picked' : ''}">
    ...
    <td class="num">${fmt(f.rutaKm, 2)}</td>
  ...
  $$('#detalle tbody tr[data-nodo]').forEach((tr) => {
    tr.addEventListener('click', () => {
      ...
      Mapa.fijar({
        ...
        nodos: [fila.sitio],
        enlaces: [{ a: res.cand, b: fila.sitio, medio: 'mw', principal: true }],
      });
    });
  });
  const btn = $('#csv-nodos');
...
function renderEnlace(res) {
  ...
  const cercaA = vecinos(A.lat, A.lon, 3);
  const cercaB = vecinos(B.lat, B.lon, 3);
  $('#detalle').innerHTML = `
    ${panelVano(A, B, mw, P)}
    <div class="panel">
      <div class="panel-head"><h3>Nodos de red próximos a cada extremo</h3>
        <div class="spacer"></div><span class="hint">para agregación o para partir el salto</span></div>
      <div class="tablewrap"><table>
        <thead><tr><th>Extremo</th><th class="num">Dist.</th><th>Código</th><th>Nombre</th>
          <th>Comuna</th><th class="num">Alt.</th><th>Tec.</th><th>FO probable</th></tr></thead>
        <tbody>${[['A', A, cercaA], ['B', B, cercaB]].flatMap(([et, ext, lista]) =>
          lista.map(({ sitio, km }) => {
            const p = probabilidadFO(sitio);
            return `<tr><td class="strong">${et}</td><td class="num">${fmt(km, 2)}</td>
              <td class="strong">${esc(sitio.id)}</td><td class="name">${esc(sitio.nombre)}</td>
              <td>${esc(sitio.comuna || '—')}</td><td class="num">${sitio.alt ? fmt(sitio.alt, 0) : '—'}</td>
              <td><span class="techs">${TECH_BITS.map(([t, b]) =>
                `<span class="${sitio.techMask & b ? 'on' : ''}">${t}</span>`).join('')}</span></td>
              <td><span class="dot ${p.score >= 60 ? 'ok' : p.score >= 35 ? 'warn' : 'bad'}"></span>${p.nivel}</td></tr>`;
          })).join('')}</tbody>
      </table></div>
    </div>`;
  ...
  Mapa.fijar({
    ...
    nodos: [...cercaA, ...cercaB].map((v) => v.sitio),
    enlaces: [{ a: A, b: B, medio: 'mw', principal: true }],
  });
...
function exportarNodos(res) {
  const cab = ['candidato', 'lat_cand', 'lon_cand', 'nodo', 'nombre_nodo', 'comuna', 'region',
    'dist_km', 'azimut', 'alt_ant_nodo', 'tecnologias', 'fo_prob', 'fo_tendido_km', 'fo_costo_usd',
  ...
  const filas = res.filas.map((r) => {
    ...
    return [res.cand.nombre, dec(res.cand.lat, 6), dec(res.cand.lon, 6), s.id, s.nombre, s.comuna, s.region,
      ...
      techList(s.techMask).join('+'), f.fo.nivel, dec(f.rutaKm, 3), Math.round(f.costo),
...
const CAMPOS_PARAM = [
  ...
  ['Fibra óptica', [
    ['sinuosidadUrbana', 'Sinuosidad urbana', 'number', { min: 1, max: 2.5, step: 0.05 }],
...
function arrancar() {
  ...
  $('#ejemplo').addEventListener('click', () => {
    ...
    $('#lote').value = [
      ...
      'Planta Til Til; -33.08120; -70.92750',
      'Radio Base Curacaví; -33.40560; -71.14210; 36',
      'Nodo Los Andes; -32.83370; -70.59810',
